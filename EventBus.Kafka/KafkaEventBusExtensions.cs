...
public static class KafkaEventBusExtensions
{
    ...
    public static void ConfigureKafkaProducer(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSingleton<IProducer<string, MessageEnvelop>>(sp =>
        {
            var producerBuilder = new ProducerBuilder<string, MessageEnvelop>(config)
                .SetValueSerializer(new MessageEnvelopSerializer());
    ...
    public static void AddKafkaMessageEnvelopConsumer(this IServiceCollection services, IConfiguration configuration, string groupId, string connectionName = "kafka")
    {
        ...
        services.AddSingleton<IConsumer<string, MessageEnvelop>>(sp =>
        {
            return new ConsumerBuilder<string, MessageEnvelop>(consumerConfig)
                .SetValueDeserializer(new MessageEnvelopDeserializer())
                .Build();
        });
    }
