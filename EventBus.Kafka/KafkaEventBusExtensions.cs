...
namespace EventBus.Kafka
{
    public static class KafkaEventBusExtensions
    {
        public static IHostApplicationBuilder AddKafkaProducer(this IHostApplicationBuilder builder, string connectionName)
        {
            builder.AddKafkaProducer<string, MessageEnvelop>(connectionName,
                ...
                {
                    builder.SetValueSerializer(new MessageEvenlopSerializer());
                });
        ...
        public static void AddKafkaEventPublisher(this IHostApplicationBuilder builder, string? topic)
        {
            ...
            if (!string.IsNullOrWhiteSpace(topic))
            {
                builder.Services.AddTransient<IEventPublisher>(
                    services => new KafkaEventPublisher(
                        topic,
        ...
        {
            builder.AddKafkaConsumer<string, MessageEnvelop>(connectionName, configureSettings: (setting) =>
            {
                setting.Config.GroupId = groupId;
                setting.Config.AutoOffsetReset = AutoOffsetReset.Earliest;
            },
            configureBuilder: (builder) =>
            {
    ...
    internal class MessageEvenlopSerializer : ISerializer<MessageEnvelop>
    {
